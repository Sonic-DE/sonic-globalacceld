    ...
    for (const QString &fileName : desktopFiles) {
        ...
        auto migrateTo = [this, componentName](KConfigGroup &group, const QString &actionName, const QString displayName) {
            ...
            if (oldShortcut != defaultShortcut) {
                _config.group("services").group(componentName).writeEntry(actionName, oldShortcut);
            }

            _config.group(componentName).writeEntry(actionName, QStringList{oldShortcut, defaultShortcut, displayName});
