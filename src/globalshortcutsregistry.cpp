    ...
    for (int i = 1; i <= dataCount; ++i) {
        ...
        for (int i = 0; i < actionsCount; ++i) {
            ...
            if (type == QLatin1String("COMMAND_URL")) {
                ...
                exec = QStringLiteral("qdbus %1 %2 %3")
                           .arg(actionGroup.readEntry("RemoteApp"),
                                actionGroup.readEntry("RemoteObj"),
                                actionGroup.readEntry("Call"),
                                actionGroup.readEntry("Arguments"));
            }
        }
        ...
        QString key = _config.group(QStringLiteral("khotkeys")).readEntry(uuid).split(QLatin1Char(',')).first();
