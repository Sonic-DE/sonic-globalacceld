...
bool GlobalShortcutsRegistry::keyPressed(int keyQt)
{
    ...
    case Qt::Key_Super_R:
    case Qt::Key_Meta:
        m_lastPressedModifiers = Utils::keyToModifier(key) | modifiers;
        return false;
    default:
        m_lastPressedModifiers = Qt::NoModifier;
        return handleNewKey(keyQt);
    }
}

bool GlobalShortcutsRegistry::handleNewKey(int keyQt)
{
...
bool GlobalShortcutsRegistry::keyReleased(int keyQt)
{
    ...
    case Qt::Key_Control:
    case Qt::Key_Alt:
        modifiers &= ~Utils::keyToModifier(key);
        constexpr auto releaseTimeout = std::chrono::milliseconds(200);
        const auto currentTime = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now().time_since_epoch());
        ...
        if (!modifiers && (currentTime - m_modifierFirstReleaseTime) < releaseTimeout) {
            handled = handleNewKey(m_lastPressedModifiers);
            m_lastPressedModifiers = Qt::NoModifier;
        }
    }
    if (m_lastShortcut) {
...
bool GlobalShortcutsRegistry::pointerPressed(Qt::KeyboardModifiers modifiers, Qt::MouseButtons pointerButtons)
{
    Q_UNUSED(modifiers)
...
bool GlobalShortcutsRegistry::axisTriggered(Qt::KeyboardModifiers modifiers, int axis)
{
    Q_UNUSED(modifiers)
    Q_UNUSED(axis)
    m_isReleasingModifierOnly = false;
